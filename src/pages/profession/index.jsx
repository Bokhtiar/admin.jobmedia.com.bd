...
export const ProfessionList = () => {
        ...
        },
        {
            ...
            cell: (row) => (
                <div className="flex gap-1">
                    <Link to={`/dashboard/profession/edit/${row._id}`}>
    ...
    const fetchData = useCallback(
        async (page) => {
            try {
                setLoading(true);
                const response = await NetworkServices.Profession.index({
                    page,
                    limit: perPage,
                });
    ...
    const destroy = async (id) => {
        try {
            const response = await NetworkServices.Profession.destroy(id)
    ...
    return <section>
        <div className="flex justify-between shadow-md p-4 px-6 rounded-md bg-white mb-3">
            <h2 className=" font-semibold text-xl">Profession List</h2>
            <Link to="/dashboard/profession/create">
