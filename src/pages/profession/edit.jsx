...
export const ProfessionEdit = () => {
    ...
    const fetchData = useCallback(async () => {
        try {
            const response = await NetworkServices.Profession.show(id)
    ...
    const onSubmit = async (data) => {
        try {
            ...
            const response = await NetworkServices.Profession.update(id, payload)
            if (response.status === 200) {
                navigate('/dashboard/profession')
                ...
                <form className="px-4" onSubmit={handleSubmit(onSubmit)}>
                    ...
                    <div>
                        ...
                        <TextInput
                            ...
                            rules={{ required: "Subject Profession is required" }}
                        />
                    </div>

                    {/* submit button */}
                    <div className="my-4 flex justify-center">
                        <PrimaryButton loading={loading} name="Profession Update"></PrimaryButton>
                    </div>
